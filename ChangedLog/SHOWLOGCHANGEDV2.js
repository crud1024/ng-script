...
class DataChangeComparator {
  ...
  constructor(options = {}) {
    ...
    this.state = {
      ...
      changeData: null,
      changeBillDate: null,
      changeProjectManager: null, // 新增：重大变更的项目负责人
      hasCompared: false,
      currentUser: null,
      isInitialized: false,
      targetElement: null,
  ...
  setupFormChangeListener() {
    ...
    useValuesChange(({ args }) => {
      ...
      this.state.changeProjectManager = null; // 新增：重置项目负责人
  ...
  processData(res, dataType) {
    ...
    try {
      ...
      // 如果是重大变更数据，提取bill_dt和u_pro_manager
      if (dataType === "重大变更") {
        if (data[0].bill_dt) {
          this.state.changeBillDate = data[0].bill_dt;
          console.log(`提取到重大变更的bill_dt: ${this.state.changeBillDate}`);
        }

        // 新增：提取重大变更的项目负责人
        if (extendObjects && extendObjects.u_pro_manager) {
          this.state.changeProjectManager = extendObjects.u_pro_manager;
          console.log(
            `提取到重大变更的项目负责人: ${this.state.changeProjectManager}`,
          );
        }
      }
      ...
      console.error(`解析${dataType}数据失败:`, error, "原始数据:", res.data);
      return null;
    }
  }
  ...
  addChangedRecords(changedFields) {
    ...
    // 使用重大变更的bill_dt作为变更日期，u_pro_manager作为变更人
    const changeDate = this.state.changeBillDate || this.getBillDate();

    // 新增：获取变更人信息（优先使用重大变更的项目负责人，其次使用当前用户）
    let changeOperator = "";
    let changeOperatorId = "";
    let changeOperatorName = "";

    if (this.state.changeProjectManager) {
      // 如果重大变更中有项目负责人，使用它
      changeOperator = this.state.changeProjectManager;
      // 尝试解析姓名和ID（假设格式为"姓名(ID)"）
      const match = changeOperator.match(/(.+)\((.+)\)/);
      if (match) {
        changeOperatorName = match[1].trim();
        changeOperatorId = match[2].trim();
      } else {
        changeOperatorName = changeOperator;
        changeOperatorId = "";
      }
    } else {
      // 否则使用当前登录用户
      changeOperatorName = this.state.currentUser.userName || "";
      changeOperatorId = this.state.currentUser.userID || "";
      changeOperator = changeOperatorId
        ? `${changeOperatorName}(${changeOperatorId})`
        : changeOperatorName;
    }
    ...
    const rowsToAdd = changedFields.map((field) => ({
      ...
      phid_emp: changeOperatorId, // 使用重大变更项目负责人的ID或当前用户ID
      u_user: changeOperatorName, // 使用重大变更项目负责人的姓名或当前用户姓名
  ...
  createTableRow(field, index) {
    ...
    // 获取变更状态
    const changeStatus = this.getChangeStatus(field.oldValue, field.newValue);
    ...
    const cells = [
      ...
      this.getOperatorDisplayName(), // 使用重大变更的项目负责人
      this.getChangeDateDisplay(), // 使用重大变更的bill_dt
      this.createStatusBadge(changeStatus),
    ];

    // 添加单元格
    cells.forEach((content, cellIndex) => {
      const td = this.createTableCell(content, cellIndex, field, changeStatus);
      row.appendChild(td);
    });
  ...
  createOperatorCell() {
    // 使用重大变更的项目负责人
    let operatorName = "";
    let fullOperatorInfo = "";

    if (this.state.changeProjectManager) {
      operatorName = this.state.changeProjectManager;
      fullOperatorInfo = this.state.changeProjectManager;
    } else {
      operatorName = this.state.currentUser.userName || "";
      fullOperatorInfo = `${this.state.currentUser.userName} (${this.state.currentUser.userID})`;
    }
  ...
  getOperatorDisplayName() {
    // 使用重大变更的项目负责人
    if (this.state.changeProjectManager) {
      return this.state.changeProjectManager;
    }
