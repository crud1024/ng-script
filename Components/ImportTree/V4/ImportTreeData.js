class NewTreeStructureGenerator {
  ...
  showImportDialog(headers, data) {
    ...
    switchContainer.style.minWidth = "100px";
    ...
    switchLabel.textContent = this.treeModeEnabled ? "树结构" : "平级";
    ...
    // 开关切换事件
    const switchButton = antSwitch.querySelector(".new-tree-switch-button");
    const switchHandle = antSwitch.querySelector(".new-tree-switch-handle");
    ...
    const toggleSwitch = (enabled) => {
      ...
      if (enabled) {
        antSwitch.classList.add("new-tree-switch-checked");
        switchButton.style.transform = "translateX(18px)";
        switchLabel.textContent = "树结构";
      } else {
        antSwitch.classList.remove("new-tree-switch-checked");
        switchButton.style.transform = "translateX(2px)";
        switchLabel.textContent = "平级";
      }
  ...
  createAntSwitch(checked = false) {
    ...
    switchButton.className = "new-tree-switch-button";
    ...
    // 设置按钮位置
    if (checked) {
      switchHandle.style.transform = "translateX(22px)";
    }
    ...
    switchContainer.addEventListener("mouseenter", () => {
      if (!switchContainer.classList.contains("new-tree-switch-checked")) {
