class NewTreeStructureGenerator {
  ...
  createLevelSection(headers, autoMatchedLevelField, isTreeConfig, sheetIndex) {
    ...
    switchCheckbox.style.cssText = `
      ...
      background-color: ${isTreeConfig && this.options.defaultTreeMode ? "#1890ff" : "rgba(0, 0, 0, 0.25)"};
    ...
    switchHandle.style.cssText = `
      position: absolute;
      top: 2px;
      left: ${isTreeConfig && this.options.defaultTreeMode ? "24px" : "2px"};
      width: 18px;
      height: 18px;
      border-radius: 18px;
      ...
      pointer-events: none;
    `;
    switchCheckbox.parentElement?.appendChild(switchHandle);

    // 重新组织开关结构
    const antSwitch = document.createElement("div");
    antSwitch.style.cssText = `
      position: relative;
      display: inline-block;
      width: 44px;
      height: 22px;
    `;
    antSwitch.appendChild(switchCheckbox);
    antSwitch.appendChild(switchHandle);
    ...
    switchCheckbox.addEventListener("change", (e) => {
      ...
      // 更新开关样式
      e.target.style.backgroundColor = canEnable
        ? "#1890ff"
        : "rgba(0, 0, 0, 0.25)";
      const handle = e.target.nextSibling;
      if (handle) {
        handle.style.transform = canEnable
          ? "translateX(22px)"
          : "translateX(0px)";
        handle.style.left = canEnable ? "24px" : "2px";
      }
    ...
    for (const config of allConfigs) {
      try {
        ...
        if (config.treeModeEnabled && config.isTree) {
          ...
          console.log(
            `Sheet "${config.sheetName}" 生成树形平级数据:`,
            parsedData,
          );
  ...
  generateTreeData(data, fieldMappings, levelField, separator) {
    const headers = data[0];
    const rows = data.slice(1);
    const treeMap = new Map(); // 存储路径到treeId的映射
    const treeData = [];
    const fieldIndexes = {};
    ...
    // 按层级路径排序，确保父节点先于子节点处理
    const sortedRows = [...rows].sort((a, b) => {
      const pathA = String(a[levelIndex] || "").trim();
      const pathB = String(b[levelIndex] || "").trim();
    ...
    sortedRows.forEach((row, rowIndex) => {
      ...
      // 存储当前节点的完整路径到treeId的映射，供子节点使用
      treeMap.set(levelPathStr, treeId);

      console.log(
        `行${rowIndex + 1}: 路径="${levelPathStr}", 层级=${currentLevel}, 节点ID=${treeId}, 父节点ID=${treePid}`,
      );
    });
