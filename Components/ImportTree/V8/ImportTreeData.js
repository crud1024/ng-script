class NewTreeStructureGenerator {
  ...
  showImportDialog(headers, data) {
    ...
    separatorContainer.appendChild(tooltipContainer);

    // 鼠标悬停显示提示 - 确保只在非禁用状态显示
    ...
    // 确保提示框不会被父容器裁剪
    separatorContainer.addEventListener("mouseenter", () => {
      if (!separatorInput.disabled) {
        tooltipContainer.style.opacity = "1";
      }
    });

    separatorContainer.addEventListener("mouseleave", () => {
      tooltipContainer.style.opacity = "0";
    });
