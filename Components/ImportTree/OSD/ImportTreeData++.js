class NewTreeStructureGenerator {
   ...
   * @param {Array} options.levelFieldKeywords 层级字段匹配关键字数组，默认 ['序号', '编号', '编码', '代码', '代号', 'code', 'no']
   * @param {string} options.defaultLevelField 默认指定的层级字段名，如果指定则优先使用
   * @param {Object} options.style 自定义样式覆盖
   * @param {Function} options.onBeforeImport 导入前事件，接收参数(parsedData, isTreeMode)，返回值将作为新的数据导入；若返回false则中止导入
   * @param {Function} options.onAfterImport 导入后事件，接收参数(result, isTreeMode, importedData)
   */
  constructor(options = {}) {
    this.options = {
      ...
      defaultLevelField: null, // 默认指定的层级字段名
      style: {}, // 保留给未来样式覆盖
      onBeforeImport: null, // 导入前事件钩子
      onAfterImport: null, // 导入后事件钩子
      ...options,
    };
    ...
    this.currentOverlay = null; // 当前模态框遮罩
    this.currentDialog = null; // 当前对话框
   ...
   * 自动匹配层级字段
   * @param {Array} headers Excel表头数组
   * @param {string} defaultLevelField 指定的层级字段名
   * @returns {string|null} 匹配到的层级字段名，未匹配返回null
   */
  autoMatchLevelField(headers, defaultLevelField = null) {
    if (!headers || headers.length === 0) return null;

    // 1. 优先使用指定的层级字段
    if (defaultLevelField) {
      // 在表头中查找完全匹配
      for (let i = 0; i < headers.length; i++) {
        const header = headers[i];
        if (header && String(header).trim() === defaultLevelField) {
          return header;
        }
      }
      // 在表头中查找包含匹配
      for (let i = 0; i < headers.length; i++) {
        const header = headers[i];
        if (header && String(header).includes(defaultLevelField)) {
          return header;
        }
      }
    }
    ...
    // 2. 按关键字匹配（优先匹配序号、编号、编码）
    // 定义优先级：序号 > 编号 > 编码 > 代码 > 代号 > code > no
    const priorityOrder = [
      "序号",
      "编号",
      "编码",
    ...
    // 先按优先级顺序匹配
    for (const priorityKeyword of priorityOrder) {
      for (let i = 0; i < headers.length; i++) {
        const header = headers[i];
        if (!header) continue;
        const headerLower = String(header).toLowerCase();
        const keywordLower = String(priorityKeyword).toLowerCase();
   ...
   */
  closeImportDialog() {
    if (this.currentOverlay && document.body.contains(this.currentOverlay)) {
  ...
  showImportDialog(headers, data) {
    ...
    // 自动匹配层级字段（传入指定的层级字段）
    const autoMatchedLevelField = this.autoMatchLevelField(
      headers,
      this.options.defaultLevelField,
    );
    ...
    const toggleSwitch = (enabled) => {
      ...
      [levelFieldSelect, separatorInput].forEach((el) => {
        ...
        el.style.backgroundColor = enabled
          ? autoMatchedLevelField && el === levelFieldSelect
            ? "#f6ffed"
            : "white"
          : "#f5f5f5";
    ...
    const confirmBtn = this.createButton("确认导入", "primary", async () => {
      ...
      if (this.treeModeEnabled) {
        ...
        try {
          ...
          await this.importToGrid(parsedData, true);
        ...
        try {
          let parsedData = this.generateFlatData(data, fieldMappings);
          ...
          await this.importToGrid(parsedData, false);
  ...
  async importToGrid(data, isTreeMode = true) {
    try {
      ...
      this.showAlert(`导入成功！${isTreeMode ? "(层级模式)" : "(平级模式)"}`);
    } catch (error) {
