...
class OSD_Message {
  ...
  show(options) {
    ...
    if (mergedConfig.duration > 0) {
      ...
      // 使用双重 requestAnimationFrame 确保DOM已渲染并开始动画
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (progressBar && progressBar.parentNode) {
            // 设置过渡效果
            progressBar.style.transition = `width ${mergedConfig.duration}s linear`;
            // 开始动画到0%
...
// 创建全局实例（如果不存在）
if (typeof window !== "undefined" && !window.OSD_Message) {
  window.OSD_Message = new OSD_Message();
}

// 导出模块
if (typeof module !== "undefined" && module.exports) {
  module.exports = OSD_Message;
}
