/**
 * 树形结构展开控制面板（支持多Tab页签 + 直接传入元素 + 记忆用户层级自动展开新数据）
 * 提供树形结构的展开/收起控制功能，自动识别当前激活的Tab页签，或使用直接指定的元素
 * 新增功能：记忆用户选择的展开层级，并自动应用到后续动态添加的节点
 */
class NGTreeExpandPanel {
  ...
  constructor(options = {}) {
    ...
    this.state = {
      ...
      // 新增：记忆用户选择的展开层级（1表示全部收起，大于1表示展开到该层）
      rememberedLevel:
        this.config.defaultState === "collapsed"
          ...
          : this.config.defaultState === "level"
            ? this.config.defaultLevel === 1
              ? 1
              : this.config.defaultLevel - 1
            : this.config.defaultLevels, // 默认全部展开时记忆为最大层级
    };
  ...
  startAutoExpandObserver() {
    ...
    this.autoExpandObserver = new MutationObserver((mutations) => {
      ...
      if (hasNewNodes && this.state.rememberedLevel > 1) {
        // 延迟执行，确保新节点已完全渲染
        setTimeout(() => {
          this._applyRememberedLevelToContainer(container);
   ...
   * @private
   */
  _applyRememberedLevelToContainer(container) {
    if (!container) return;
    const rememberedLevel = this.state.rememberedLevel;
    if (rememberedLevel <= 1) return; // 无需展开
    ...
    rows.forEach((row) => {
      ...
      if (levelMatch) {
        ...
        // 如果当前层级小于记忆层级，且该节点有折叠图标（即尚未展开），则尝试展开
  ...
  /**
   * 更新记忆层级
   * @param {number} newLevel - 新的记忆层级（1表示全部收起，大于1表示展开到该层）
   * @private
   */
  _updateRememberedLevel(newLevel) {
    this.state.rememberedLevel = newLevel;
  }
  ...
  onLevelButtonClick(level) {
    ...
    // 新增：记忆用户选择的层级
    if (level == 1) {
      this._updateRememberedLevel(1);
      this.collapseAll(); // 收起就是一级
    } else {
      this._updateRememberedLevel(level - 1);
  ...
  onExpandButtonClick() {
    ...
    if (this.state.isExpanded) {
      ...
      console.log("执行了展开操作");
      // 新增：记忆全部展开（设为大数）
      this._updateRememberedLevel(999);
      this.expandAll();
    } else {
  ...
  onToButtonClick() {
    ...
    // 新增：记忆用户选择的自定义层级
    this._updateRememberedLevel(actualLevel);
    ...
    setTimeout(() => {
      ...
      switch (this.config.defaultState) {
        case "collapsed":
          ...
          this.state.rememberedLevel = 1; // 新增
          break;
        ...
        case "level":
          if (
            ...
            if (this.config.defaultLevel === 1) {
              ...
              this.state.rememberedLevel = this.config.defaultLevel - 1; // 新增
            }
        ...
        default:
          ...
          this.state.rememberedLevel = this.config.defaultLevels; // 新增：记忆为最大层级
          break;
      }
    }, 100);
  }
  ...
  setState(state, level) {
    ...
    switch (state) {
      ...
      case "level":
        if (level && level >= 1 && level <= this.config.maxCustomLevel) {
          if (level === 1) {
            ...
            this.state.rememberedLevel = level - 1; // 新增
          }
          this.state.currentLevel = level;
      ...
      case "expanded":
        ...
        this.state.currentLevel = this.config.defaultLevels;
        this.state.rememberedLevel = this.config.defaultLevels; // 新增
        break;
    }
  }
