/**
 * 树形结构展开控制面板（支持多Tab页签 + 直接传入元素 + 作用域隔离）
...
class NGTreeExpandPanel {
   ...
   * @param {HTMLElement} [options.containerScope=null] - 容器查找作用域（限定在指定元素内查找，用于多实例隔离）
  ...
  constructor(options = {}) {
    ...
    this._containerScope = this.config.containerScope;
  ...
  /**
   * 获取查找容器的基础作用域
   * @returns {HTMLElement|Document} 作用域根节点
   * @private
   */
  _getSearchRoot() {
    if (this._containerScope && document.body.contains(this._containerScope)) {
      return this._containerScope;
    }
    return document;
  }
  ...
  init() {
    ...
    // 解析树形容器：
    // - 如果是 HTMLElement（用户显式传入了具体元素），直接存储，不跟随 Tab 切换
    // - 如果是字符串选择器，不在此处解析，留给 _getCurrentContainer() 每次动态
    //   查找当前激活 Tab 中的容器，确保切换 Tab 后操作的是正确的容器
    if (this.config.containerSelector instanceof HTMLElement) {
      this._containerElement = this.config.containerSelector;
    } else {
      // 字符串选择器 — 不提前解析，让 _getCurrentContainer() 动态查找
      this._containerElement = null;
    }
  ...
  _getCurrentContainer() {
    ...
    // 获取查找作用域
    const searchRoot = this._getSearchRoot();
    ...
    for (const container of containers) {
      // 查找最近的激活的tab页签（支持 ant-tabs-tabpane 和 ant-tabs-tab-active）
      let isInActiveTab = false;

      // 方式1：查找 ant-tabs-tabpane 激活容器
      ...
      // 方式2：如果没找到，尝试查找激活的 ant-tabs-tab
      if (!isInActiveTab) {
        // 向上查找所有祖先，看是否在某个激活的 tab 内容区域内
        let parent = container.parentElement;
        while (parent && parent !== searchRoot) {
          // 检查是否是 ant-tabs-tabpane
          if (
            parent.classList &&
            parent.classList.contains("ant-tabs-tabpane")
          ) {
            const isActive =
              parent.classList.contains("ant-tabs-tabpane-active") &&
      ...
      // 方式3：检查容器是否在可见的 tab 面板中（通过样式判断）
      if (!isInActiveTab) {
        // 查找所有 ant-tabs-tabpane，找到可见的那个
        const searchRootLocal = this._getSearchRoot();
        const allTabPanes =
          searchRootLocal.querySelectorAll(".ant-tabs-tabpane");
        for (const pane of allTabPanes) {
          const isActive =
            pane.classList.contains("ant-tabs-tabpane-active") &&
            getComputedStyle(pane).display !== "none";
          if (isActive && pane.contains(container)) {
            isInActiveTab = true;
            break;
          }
        }
      ...
      if (
        !container.closest(".ant-tabs-tabpane") &&
        !container.closest(".ant-tabs-tab")
      ) {
        return containers[0];
      }
    }
  ...
  createPanel() {
    ...
    panelBody.id = `tree-expand-panel-body-${instanceId}`;
