...
class NGTreeExpandPanel {
  ...
  constructor(options = {}) {
    ...
    this.config = {
      ...
      containerScope: null, // 新增：容器查找作用域
      animationDelay: 100,
      position: {
        index: 0,
        side: "after",
    ...
    this._containerScope = this.config.containerScope; // 存储作用域
  ...
  _getCurrentContainer() {
    ...
    // 否则根据选择器在作用域内查找所有匹配的容器，并找到位于激活Tab页签中的容器
...
const NGTreeExpandPanelExample = {
  ...
  /**
   * 带作用域的选择器方式（推荐用于多页面/多实例场景）
   * @param {HTMLElement} scope - 作用域根元素
   * @param {Object} options - 配置选项
   * @returns {NGTreeExpandPanel} 组件实例
   */
  withScope: function (scope, options = {}) {
    const defaultOptions = {
      defaultLevels: 5,
