...
class WebLayoutBuilder {
  ...
  addItem(options = {}) {
    const defaultItem = {
      ...
      zIndex: 1, // z-index层级
      bindElement: options.bindElement || null, // 绑定的DOM元素或HTML字符串
      renderMode: options.renderMode || "default", // 渲染模式: 'default', 'html', 'element'
    };
  ...
  createContentElement(item) {
    ...
    switch (item.renderMode) {
      case "html":
        // HTML字符串模式
        if (typeof item.bindElement === "string") {
          contentElement.innerHTML = item.bindElement;
      ...
      case "element":
        // DOM元素模式
        if (item.bindElement instanceof HTMLElement) {
          // 清空并添加绑定的元素
          contentElement.innerHTML = "";
          contentElement.appendChild(item.bindElement);

          // 确保元素填充内容区域
          item.bindElement.style.width = "100%";
          item.bindElement.style.height = "100%";
          item.bindElement.style.boxSizing = "border-box";
        } else if (typeof item.bindElement === "string") {
          // 如果是选择器字符串，查找元素
          try {
            const element = document.querySelector(item.bindElement);
            if (element) {
              ...
              contentElement.textContent = item.content || "元素不存在";
            }
          } catch (error) {
            console.error("绑定元素错误:", error);
            contentElement.textContent = item.content || "绑定错误";
          }
        }
        break;
  ...
  exportLayout() {
    return {
      config: { ...this.config },
      items: this.layoutItems.map((item) => ({
        ...item,
        // 注意：DOM元素无法被序列化，需要特殊处理
        bindElement: this.serializeBindElement(item.bindElement),
      })),
      version: "2.0",
    };
  }
  ...
  // 序列化绑定的元素（用于导出）
  serializeBindElement(element) {
    if (!element) return null;

    if (typeof element === "string") {
      return element; // HTML字符串或选择器字符串
    } else if (element instanceof HTMLElement) {
      // 如果是DOM元素，返回其outerHTML
  ...
  // 绑定元素到布局项
  bindElement(itemId, element, options = {}) {
    const item = this.layoutItems.find((item) => item.id === itemId);
    if (!item) return false;

    const renderMode = options.renderMode || "element";
  ...
  // 绑定HTML到布局项
  bindHTML(itemId, htmlString, options = {}) {
    return this.bindElement(itemId, htmlString, {
      ...options,
      renderMode: "html",
    });
  }
  ...
  // 更新绑定元素的内容
  updateElementContent(itemId, content) {
    const item = this.layoutItems.find((item) => item.id === itemId);
    if (!item) return false;

    if (item.renderMode === "html" || item.renderMode === "default") {
      item.content = content;
    } else if (item.renderMode === "element") {
      // 如果绑定的是DOM元素，更新其内容
      if (item.bindElement instanceof HTMLElement) {
        item.bindElement.innerHTML = content;
      } else {
        item.content = content;
      }
    }
    ...
    const itemElement = document.getElementById(itemId);
    if (itemElement) {
      const contentElement = itemElement.querySelector(".layout-item-content");
      if (contentElement) {
        // 只更新内容区域，而不是整个项
        if (item.renderMode === "html") {
...
/*
    ...
    const item2Id = layoutTool.addItem({
        ...
        title: 'HTML内容区',
        content: '初始HTML内容',
        renderMode: 'html' // HTML模式
    });
    
    // 3. 绑定HTML字符串
    layoutTool.bindHTML(item2Id, '<div style="padding: 20px;"><h3>HTML标题</h3><p>这是HTML内容</p><button>点击我</button></div>');
    
    // 4. 创建DOM元素并绑定
    const customElement = document.createElement('div');
    customElement.innerHTML = '<h4>动态内容</h4><p>这个元素是动态创建的</p>';
    customElement.style.padding = '20px';
    customElement.style.color = 'blue';
    
    const item3Id = layoutTool.addItem({
        x: 1, y: 7,
        width: 6, height: 6,
        title: 'DOM元素绑定',
        content: '绑定DOM元素',
    ...
    layoutTool.bindElement(item3Id, customElement);
    
    // 5. 绑定现有DOM元素（通过选择器）
    // 假设页面上有 <div id="existing-content">已有内容</div>
    layoutTool.bindElement(item3Id, '#existing-content');
    
    // 6. 更新元素内容
    layoutTool.updateElementContent(item1Id, '更新后的文本内容');
    layoutTool.updateElementContent(item2Id, '<div>更新后的HTML</div>');
