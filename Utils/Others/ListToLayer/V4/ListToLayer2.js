...
class ListToLayer {
   ...
   * @param {boolean} options.versionCompare - 是否启用版本号比较（如1.1, 1.2, 1.10），默认true
   */
  constructor(options = {}) {
    // 默认配置
    this.defaultOptions = {
      ...
      versionCompare: true, // 新增：是否启用版本号比较
    };
    ...
    this.versionCompare = this.config.versionCompare;
  }
  ...
  /**
   * 检查是否为版本号格式（如 1.1, 1.2.3, 2.1.10 等）
   * @param {*} value - 要检查的值
   * @returns {boolean}
   */
  isVersionFormat(value) {
    if (typeof value !== "string") return false;
    // 匹配版本号格式：数字开头，可包含多个点和数字，如 1.1, 1.10.2, 2.1.10.20
   ...
   * @returns {Array|null} 版本号数字数组，解析失败返回null
   */
  parseVersion(version) {
    if (!this.isVersionFormat(version)) return null;
  ...
  smartCompare(a, b) {
    ...
    if (this.versionCompare) {
      // 检查是否为字符串类型的版本号
      const aIsVersion = typeof a === "string" && this.isVersionFormat(a);
      const bIsVersion = typeof b === "string" && this.isVersionFormat(b);

      if (aIsVersion || bIsVersion) {
        return this.compareVersion(String(a), String(b));
      }
    }
    ...
    if (
      !isNaN(aNum) &&
      !isNaN(bNum) &&
      String(a) === String(aNum) &&
      String(b) === String(bNum)
    ) {
      return aNum - bNum;
    }
  ...
  defaultSort(nodes) {
    ...
    const sorted = [...nodes].sort((a, b) => {
      ...
      if (aParent !== bParent) {
        ...
        return this.smartCompare(aVal, bVal);
      }

      // parentKey 相同的情况下，按 idKey 排序（使用智能比较）
      const aId = a[this.idKey];
      const bId = b[this.idKey];
      return this.smartCompare(aId, bId);
    });
  ...
  customSort(nodes) {
    ...
    return [...nodes].sort((a, b) => {
      for (const rule of this.sortRules) {
        ...
        // 使用智能比较
        let compareResult = this.smartCompare(aVal, bVal);
  ...
  convert(list, options = {}) {
    ...
    const tempVersionCompare =
      options.versionCompare !== undefined
        ? options.versionCompare
        : this.versionCompare;

    // 临时覆盖版本比较设置
    const originalVersionCompare = this.versionCompare;
    if (options.versionCompare !== undefined) {
      this.versionCompare = options.versionCompare;
    }

    // 如果没有数据，返回空数组
    if (!list || !Array.isArray(list) || list.length === 0) {
      // 恢复原设置
      this.versionCompare = originalVersionCompare;
      return [];
    }
