/**
 * DynamicTemplateExporter V8 - 动态模板导出器
 *
 * V8 变更：
 *   - 明细表按 _dN 后缀序号升序排列（d1 → d2 → d3 ...），主表排在最前
 *
 ...
 * 页面中调用：
 *   $NG.loadScript(
 *     "https://fastly.jsdelivr.net/gh/crud1024/ng-script@main/Utils/Others/ExportModle/V7/DynamicTemplateExporter.js",
 *     async function () {
 *       const exporter = new DynamicTemplateExporter(APIresult, { includeMaster: false });
 *       await exporter.export();
 *     }
 *   );
 */
...
class DynamicTemplateExporter {
  ...
  extractTables() {
    ...
    // V8: 按 d 表序号增序排列
    // 主表排在最前面，明细表按 _dN 后缀数字升序（d1 → d2 → d3 ...）
    tables.sort(function (a, b) {
      var indexA = self.extractDetailIndex(a.tableName);
      var indexB = self.extractDetailIndex(b.tableName);
  ...
  /**
   * 从明细表名中提取 d 序号
   * 例如 "inv_list_d1" → 1, "inv_list_d2" → 2
   * 非明细表（无 _dN 后缀）返回 null
   */
  extractDetailIndex(tableName) {
    var match = tableName.match(/_d(\d+)$/);
    if (match) {
  ...
  /**
   * 生成一个工作表
   * V7 修复：表头样式使用 for 循环显式 getCell 确保所有列都被应用
   */
  async generateWorksheet(workbook, sheetConfig, sheetIndex) {
    ...
    // 第一步：设置列定义（先设列宽，不设 header，避免自动创建 header 行被覆盖）
    ...
    // 第二步：显式遍历所有列设置表头样式 [V7 修复]
    // 使用 for 循环逐列 getCell，确保每个单元格都被创建并设置
    var headerRow = worksheet.getRow(1);
    var headerStyle = this.getHeaderStyle();
    headerRow.height = 22;

    for (var ci = 1; ci <= columns.length; ci++) {
      var cell = headerRow.getCell(ci);
