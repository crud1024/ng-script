class ExcelTemplateExporter {
  ...
  async startExport() {
    try {
      ...
      if (
        !this.options.schema.columns ||
        !Array.isArray(this.options.schema.columns)
      ) {
        throw new Error("schema.columns 必须是一个数组");
      }
  ...
  flattenColumns(columns, parentHeader = "") {
    ...
    if (!columns || !Array.isArray(columns)) {
      console.warn("flattenColumns: columns 不是数组，返回空数组", columns);
      return [];
    }
    ...
    for (const col of columns) {
      ...
      if (col.columns && Array.isArray(col.columns) && col.columns.length > 0) {
        const nestedColumns = this.flattenColumns(
          col.columns,
          col.header || parentHeader,
        );
        result.push(...nestedColumns);
      } else if (col.dataIndex || col.header) {
        // 普通列，直接添加（确保有 dataIndex 或 header）
        result.push({
          ...col,
          parentHeader: parentHeader,
          // 如果有父级表头，合并显示
          fullHeader: parentHeader
            ? `${parentHeader} - ${col.header || col.dataIndex}`
  ...
  getProcessedHeader(column) {
    // 使用完整表头（包含父级）
    let originalHeader =
      column.fullHeader || column.header || column.dataIndex || "";
      ...
      if (
        ...
        if (validValues.length > 0) {
          ...
          try {
            ...
            console.warn(`设置数据验证失败: ${validationAddress}`, e);
          }
        }
      }
    });
