...
class BatchPDFToPNGConverter {
   ...
   * @param {string} options.logo 水印（所有单据使用相同的logo）
  ...
  async initializeAndStart() {
    ...
    try {
      ...
      // 显示水印信息（调试用）
      console.log(`使用水印: ${this.options.logo || "无水印"}`);
   ...
   */
  showInitialMessage() {
    const message = `开始批量处理 ${this.options.dataList.length} 个单据，一级文件夹：${this.options.parentFolderName}，水印：${this.options.logo || "无"}`;
    console.log(message);
    if (
  ...
  async processSingleDocument(item, index) {
    ...
    try {
      // 构建单个文件的URL（使用构造函数中传入的logo）
      const url = this.buildSingleFileUrl(item);
      console.log(`[${index + 1}] 下载URL:`, url);
      console.log(`[${index + 1}] 使用水印: ${this.options.logo || "无水印"}`);
  ...
  /**
   * 构建单个文件的URL（修复：所有单据使用相同的水印）
   */
  buildSingleFileUrl(item) {
    // 使用构造函数中传入的logo参数，确保所有单据使用相同的水印
...
处理时间: ${duration}秒`;
  ...
  getStatus() {
    return {
      ...
      errors: this.errors,
      logo: this.options.logo, // 返回当前使用的水印
    };
  }
}
