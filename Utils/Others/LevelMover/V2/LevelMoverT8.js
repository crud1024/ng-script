class TreeTableManager {
  ...
  handleUpgrade(table, row, parentRow) {
    ...
    // 使用深拷贝保留所有字段
    const rowCopy = this.deepCloneNode(row);
  ...
  // ========== 修改：handleDowngrade 使用深拷贝并保留子节点 ==========
  handleDowngrade(table, rows, row) {
    ...
    if (row && this.isRootNode(row)) {
      ...
      if (row.children && row.children.length > 0) {
        rowCopy.children = this.deepCloneNode(row.children);
      }
