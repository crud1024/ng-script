class TreeTableManager {
  ...
  preprocessTreeData(data) {
    ...
    const processNode = (node) => {
      ...
      // 修复根节点的 pid：将 null 或 undefined 转换为 "0"
  ...
  isValidTreeStructure(tempRows) {
    ...
    // 找根节点
    const rootNode = tempRows.find((node) => this.isRootNode(node));
    if (!rootNode || !rootNode.s_tree_id) {
      console.log("失败：找不到有效的根节点");
      return false;
    }
    // 修复根节点 pid
  ...
  normalizeTree(data) {
    ...
    const buildMap = (node) => {
      if (!node) return;
      nodeMap.set(node.s_tree_id, node);
      if (Array.isArray(node.children)) {
        node.children.forEach((child) => buildMap(child));
      }
    };
    data.forEach((node) => buildMap(node));
