class TreeTableManager {
    ...
    for (const key of allKeys) {
      ...
      if (
        excludeKeys.some(
          (ex) =>
            keyStr === ex ||
            keyStr.includes("treeParent") ||
            keyStr.includes("__parent"),
        )
      ) {
        continue;
      }
  ...
  // ========== 修改：cloneNode 使用安全深拷贝 ==========
  cloneNode(node) {
    if (!node) return null;
    return this.safeDeepClone(node);
  }

  // ========== 修改：handleUpgrade 使用 cloneNode ==========
  handleUpgrade(table, row, parentRow) {
    ...
    const removed = removeNode(data);
    if (!removed) {
      for (let i = 0; i < data.length; i++) {
        if (data[i].s_tree_id === targetId) {
          data.splice(i, 1);
          break;
        }
      }
  ...
  // ========== 修改：handleDowngrade 使用 cloneNode ==========
  handleDowngrade(table, rows, row) {
    ...
    const targetId = selectedRow.s_tree_id;
