class TreeTableManager {
  ...
  mergeConfig(config) {
    const defaultConfig = {
      ...
      customIcons: {},
      buttonMapping: {}, // 自定义按钮映射 { tableName: { up: 'custom_up_id', ... } }
    };
  ...
  // ==================== 获取按钮配置 ====================
  getButtonConfig(tableName, action) {
    const context = this.getTableContext(tableName);
    return context.buttonConfigs[action] || null;
  }

  // ==================== 更新按钮文本 ====================
  updateButtonText(tableName, action, text) {
    const context = this.getTableContext(tableName);
    if (context.buttonConfigs[action]) {
      context.buttonConfigs[action].text = text;
    }
  }

  // ==================== 更新按钮图标 ====================
  updateButtonIcon(tableName, action, iconSvg) {
    const context = this.getTableContext(tableName);
    if (context.buttonConfigs[action]) {
      context.buttonConfigs[action].iconSvg = iconSvg;
    }
  }
  ...
  updateTableContext(tableName) {
    const context = this.getTableContext(tableName);
    const table = $NG.getCmpApi(tableName);
    context.table = table;
  ...
  initAllTableListeners() {
    this.config.tables.forEach((tableName) => {
      this.initTableListener(tableName);
    });
  }

  initTableListener(tableName) {
    $NG.getCmpApi(tableName).subscribe(({ args, table }) => {
      const context = this.getTableContext(tableName);
      context.table = table;
      context.selectedIndex = table._lastHighlightRowIndex || 0;
      context.rows = table.getSelectedData();
  ...
  registerAction(tableName) {
    ...
    useClick(async function ({ args }) {
      ...
      if (rows.length === 0) {
        $NG.message(buttonConfigs.up.tooltip || "请确认已选中分枝");
        return;
      }
    ...
    useClick(async function ({ args }) {
      ...
      if (rows.length === 0) {
        $NG.message(buttonConfigs.down.tooltip || "请确认已选中分枝");
        return;
      }
    ...
    useClick(async function ({ args }) {
      ...
      if (rows.length === 0) {
        $NG.message(buttonConfigs.rise.tooltip || "请确认已选中分枝");
        return;
      }
    ...
    useClick(async function ({ args }) {
      ...
      if (rows.length === 0) {
        $NG.message(buttonConfigs.decline.tooltip || "请确认已选中分枝");
        return;
      }
