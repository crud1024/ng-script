...
class ExcelExporter {
  ...
  constructor(options = {}) {
    ...
    // 单元格标记颜色配置
    this.markCellColor = "FFFF4D4F"; // 红色
    ...
    // d0 表名配置
    this.d0SheetName = options.d0SheetName || null;
  ...
  /**
   * 基于 parentId 方式扁平化树形数据（适用于平铺数据但通过 pid 关联的结构）
   * @param {Array} flatData 平铺但带父子关系的数据
   * @returns {Array} 处理后的数据（会添加层级信息并按层级排序）
   */
  flattenTreeDataByParentId(flatData) {
    ...
    for (const item of flatData) {
      ...
      result.push(node);
      nodeMap.set(node.phid || node.id, node);
    }
  ...
  getBodyCellStyle(rowIndex, rowData = null, fieldName = null) {
    ...
    if (rowData && fieldName) {
      ...
      if (Array.isArray(marks) && marks.includes(fieldName)) {
        fontColor = this.markCellColor; // 使用红色
      }
    }
   ...
   * @param {Array} rowsData 数据行（可能是树形结构）
   * @returns {Worksheet} Excel工作表
   */
  createWorksheet(sheetName, filteredTree, rowsData) {
    // ========== 智能扁平化数据 ==========
    const flattenedData = this.smartFlattenData(rowsData);
    console.log(
      `${sheetName} 数据扁平化: ${rowsData?.length || 0} 行 -> ${flattenedData.length} 行`,
    );

    // 如果扁平化后没有数据，但原始有数据，打印警告
    if (rowsData && rowsData.length > 0 && flattenedData.length === 0) {
      console.warn(`${sheetName} 数据扁平化后为空，请检查数据结构`);
    }
  ...
  async export(designData, resultData, customFileName = null) {
    ...
    try {
      ...
      // ========== 处理 d0 表（使用 d1 的列模板 + d0 的数据） ==========
      if (this.d0SheetName) {
        const d0Key = Object.keys(busTableChnMap).find((key) =>
          key.endsWith("_d0"),
        );
        const d1Key = Object.keys(busTableChnMap).find((key) =>
          key.endsWith("_d1"),
        );
        let d0Data = resultData[d0Key] || resultData["d0"] || [];

        if (d1Key) {
          const d1GridConfig = configData.uiContent?.grid?.[d1Key];

          if (d1GridConfig) {
            const rawColumns = d1GridConfig.columns || [];
            const filteredTree = this.filterColumns(rawColumns);

            console.log(
              `\n========== 处理 d0 表，Sheet名称: ${this.d0SheetName} (模板: ${d1Key}) ==========`,
            );
            console.log(`d0 原始数据行数: ${d0Data.length}`);

            // 扁平化 d0 数据
            const flattenedD0Data = this.smartFlattenData(d0Data);
            console.log(`d0 扁平化后数据行数: ${flattenedD0Data.length}`);
            ...
            if (filteredTree.length > 0) {
              ...
              console.log(`d0 Sheet 创建完成`);
            } else {
              console.warn("d0 无有效列，跳过");
            }
          } else {
            console.warn(`未找到 d1 表格配置 (${d1Key})，d0 跳过`);
          }
        } else {
          console.warn("未找到 d1 表配置，d0 跳过");
        }
      }
      ...
      for (const tableKey of detailTables) {
        ...
        console.log(`${tableKey} 扁平化后数据行数: ${flattenedData.length}`);

        // 打印包含样式标记的行数据，用于调试
        const styledRowsCount = flattenedData.filter(
          (row) =>
            (row.u_log_type !== undefined && row.u_log_type !== null) ||
            (row.marks && row.marks.length > 0) ||
            (row.Marks && row.Marks.length > 0),
        ).length;
        console.log(`${tableKey} 包含样式标记的行数: ${styledRowsCount}`);

        // 创建工作表
        this.createWorksheet(sheetName, filteredTree, flattenedData);
...
//     d0SheetName: '投资汇总表'  // 指定 d0 表的 Sheet 名称
// });
...
//         alternateBackgroundColor: 'FFF5F5F5'
//     },
//     borderStyle: {
//         color: 'FFCCCCCC',
//         style: 'thin'
