...
class ExcelExporter {
  constructor(options = {}) {
    ...
    // 新增字段配置（使用 is_manual1）
    this.highlightConfig2 = {
      inv_xmdx_budget_d2: {
        column: "initial_est_cost",
        conditionField: "is_manual1",
      },
      inv_xmdx_budget_d3: {
        column: "initial_est_cost",
        conditionField: "is_manual1",
  ...
  getBodyCellStyle(
    ...
    // 检查是否需要高亮显示（根据 is_manual 或 is_manual1）
    ...
    if (shouldHighlight) {
      ...
      // 调试日志
      if (rowData) {
        const conditionField = this.getConditionField(tableKey, fieldName);
        console.log(
          `高亮单元格: 表=${tableKey}, 列=${fieldName}, 条件=${conditionField}=${rowData[conditionField]}`,
        );
      }
    }
    // 原有日志类型颜色处理（优先级低于高亮）
   ...
   * @param {string} fieldName - 字段名
   * @returns {string|null} 条件字段名
   */
  getConditionField(tableKey, fieldName) {
    // 先检查原有字段配置
    const config = this.highlightConfig[tableKey];
    if (config && config.column === fieldName) {
      return config.conditionField;
    }
    // 再检查新增字段配置
    const config2 = this.highlightConfig2[tableKey];
    if (config2 && config2.column === fieldName) {
      return config2.conditionField;
    }
    return null;
  }
  ...
  shouldHighlightCell(tableKey, fieldName, rowData) {
    ...
    const config2 = this.highlightConfig2[tableKey];
    if (config2 && config2.column === fieldName) {
      const conditionValue = rowData[config2.conditionField];
      return conditionValue === 1 || conditionValue === "1";
    }
  ...
  getDefaultD0Columns() {
    return [
      { header: "序号", dataIndex: "s_tree_no", width: 12 },
    ...
    try {
      ...
      if (d0Key) {
        ...
        if (filteredTree && filteredTree.length > 0) {
          // d0表跳过扁平化，tableKey 传 null（d0 不需要高亮）
      ...
      for (const tableKey of otherTables) {
        ...
        // 打印 is_manual 和 is_manual1 字段信息以便调试
        rowsData.forEach((row, idx) => {
          if (row.is_manual === 1 || row.is_manual === "1") {
            const config = this.highlightConfig[tableKey];
            if (config) {
              console.log(
                `${tableKey} 第${idx + 1}行 is_manual=1，高亮 ${config.column} 列`,
              );
            }
          }
          if (row.is_manual1 === 1 || row.is_manual1 === "1") {
            const config2 = this.highlightConfig2[tableKey];
            if (config2) {
              console.log(
                `${tableKey} 第${idx + 1}行 is_manual1=1，高亮 ${config2.column} 列`,
              );
            }
          }
        });
