...
class ExcelExporter {
  ...
  async export(designData, resultData, customFileName = null) {
    ...
    try {
      ...
      // ========== 处理 d0 表 ==========
      if (d0Key) {
        const sheetName = busTableChnMap[d0Key] || "投资汇总表";
        ...
        if (flattenedD0Data.length > 0) {
          console.log(`d0 数据示例:`, flattenedD0Data[0]);
        }
        ...
        if (filteredTree && filteredTree.length > 0) {
          this.createWorksheet(sheetName, filteredTree, flattenedD0Data);
