...
class ExcelExporter {
  ...
  constructor(options = {}) {
    ...
    // ========== 行背景色映射配置（与 RowBackgroundManager 的 colorMap 保持一致） ==========
    ...
    // ========== 单元格标记颜色配置（与 CellFontColorManager 的红色保持一致） ==========
  ...
  async export(designData, resultData, customFileName = null) {
    ...
    try {
      ...
      if (this.d0SheetName) {
        const d0Key = Object.keys(busTableChnMap).find((key) => key.endsWith("_d0"));
        const d1Key = Object.keys(busTableChnMap).find((key) => key.endsWith("_d1"));
        const d0Data = resultData[d0Key] || resultData["d0"] || [];
        ...
        if (d1Key) {
          ...
          if (d1GridConfig) {
            ...
            if (filteredTree.length > 0) {
              this.createWorksheet(this.d0SheetName, filteredTree, d0Data);
      ...
      for (const tableKey of detailTables) {
        ...
        // ========== 打印包含 u_log_type 或 marks 的行数据，用于调试 ==========
