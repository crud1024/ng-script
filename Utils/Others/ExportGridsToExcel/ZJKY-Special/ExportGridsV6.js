...
class ExcelExporter {
  ...
  // 专门为 d0 数据排序：按 s_tree_no 排序，保持父子顺序
  sortD0Data(data) {
  ...
  createWorksheet(sheetName, filteredTree, rowsData, skipFlatten = false) {
    // 确保 rowsData 是数组
  ...
  async export(designData, resultData, customFileName = null) {
    ...
    try {
      ...
      if (d0Key) {
        ...
        console.log(`d0 原始数据:`, JSON.stringify(d0Data, null, 2));

        // 关键修改1：对 d0 数据按 s_tree_no 排序，保持父子顺序
        d0Data = this.sortD0Data(d0Data);
        console.log(
          `d0 排序后数据顺序:`,
          d0Data.map((item) => `${item.s_tree_no} - ${item.s_tree_name}`),
        );
        ...
        if (filteredTree && filteredTree.length > 0) {
          // 关键修改2：d0表跳过扁平化，传入 true
          this.createWorksheet(sheetName, filteredTree, d0Data, true);
      ...
      for (const tableKey of otherTables) {
        ...
        // 其他表正常处理（可能需要扁平化）
        this.createWorksheet(sheetName, filteredTree, rowsData, false);
