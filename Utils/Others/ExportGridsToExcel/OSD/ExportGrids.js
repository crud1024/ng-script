...
class ExcelExporter {
   ...
   * @param {Object} options.styleFieldMapping 样式字段映射配置
   * @param {string} options.styleFieldMapping.logTypeField 行背景色字段名，默认 'log_type'，支持数组如 ['log_type', 'ulogType', 'u_log_type']
   * @param {string} options.styleFieldMapping.marksField 标记字段名，默认 'marks'，支持数组如 ['marks', 'Marks', 'markFields']
   */
  constructor(options = {}) {
    // 文件名配置
    ...
    // ========== 样式字段映射配置 ==========
    // 支持字符串或数组，如 'log_type' 或 ['log_type', 'ulogType', 'u_log_type']
    this.styleFieldMapping = {
      // 行背景色字段（优先级：按数组顺序查找第一个存在的字段）
      logTypeField: options.styleFieldMapping?.logTypeField || "log_type",
      // 标记字段（优先级：按数组顺序查找第一个存在的字段）
      marksField: options.styleFieldMapping?.marksField || "marks",
      ...options.styleFieldMapping,
    };

    // 标准化为数组格式，方便处理
    this.logTypeFieldNames = Array.isArray(this.styleFieldMapping.logTypeField)
      ? this.styleFieldMapping.logTypeField
      : [this.styleFieldMapping.logTypeField];

    this.marksFieldNames = Array.isArray(this.styleFieldMapping.marksField)
      ? this.styleFieldMapping.marksField
      : [this.styleFieldMapping.marksField];
    ...
    // 允许用户自定义颜色映射
    if (options.logTypeColorMap) {
      this.logTypeColorMap = {
        ...this.logTypeColorMap,
        ...options.logTypeColorMap,
      };
    }

    // ========== 单元格标记颜色配置 ==========
    this.markCellColor = options.markCellColor || "FFFF4D4F"; // 红色
  ...
  /**
   * 从行数据中获取指定字段的值（支持多个候选字段名）
   * @param {Object} rowData 行数据
   * @param {Array} fieldNames 候选字段名数组
   * @returns {any} 第一个存在的字段的值，如果都不存在返回 undefined
   */
  _getFieldValue(rowData, fieldNames) {
  ...
  /**
   * 检查字段是否在 marks 中（支持多个 marks 字段名）
   * @param {Object} rowData 行数据
  ...
  _isFieldMarked(rowData, fieldName) {
    ...
    // 获取 marks 数据（尝试多个候选字段名）
    let marksRaw = null;
    for (const marksFieldName of this.marksFieldNames) {
      const value = rowData[marksFieldName];
  ...
  getBodyCellStyle(rowIndex, rowData = null, fieldName = null) {
    ...
    // ========== 1. 优先检查行背景色（动态字段名） ==========
    const logTypeValue = this._getFieldValue(rowData, this.logTypeFieldNames);
    if (logTypeValue !== undefined && logTypeValue !== null) {
      if (this.logTypeColorMap[logTypeValue]) {
        backgroundColor = this.logTypeColorMap[logTypeValue];
      }
    }

    // ========== 2. 如果没有 log_type 背景色，再检查斑马纹 ==========
    if (
  ...
  createWorksheet(sheetName, filteredTree, rowsData) {
    ...
    for (let rowIndex = 0; rowIndex < rowsData.length; rowIndex++) {
      ...
      for (let idx = 0; idx < leaves.length; idx++) {
        ...
        // 设置对齐方式：数值右对齐，文本左对齐，整列统一
        const alignment = { vertical: "middle" };
        // 根据实际值的类型判断对齐方式（整列统一）
        alignment.horizontal = this._isNumeric(row[leaf.dataIndex])
          ? "right"
          : "left";

        // ========== 应用表体样式（包含行背景色和单元格字体颜色逻辑） ==========
  ...
  async export(designData, resultData, customFileName = null) {
    ...
    try {
      ...
      for (const tableKey of detailTables) {
        ...
        // ========== 打印包含样式标记的行数据，用于调试 ==========
        const logTypeSample = this._getFieldValue(
          rowsData[0] || {},
          this.logTypeFieldNames,
        );
        const marksSample = this._getFieldValue(
          rowsData[0] || {},
          this.marksFieldNames,
        );
        console.log(
          `${tableKey} 样式字段映射: logType字段=${this.logTypeFieldNames.join("/")}, marks字段=${this.marksFieldNames.join("/")}`,
        );
        console.log(
          `${tableKey} 示例数据 - logType值: ${logTypeSample}, marks值:`,
          marksSample,
        );
