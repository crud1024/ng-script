...
class ExcelExporter {
    ...
    if (typeof marksData === "string") {
      ...
      try {
        ...
        try {
          ...
          console.log("解析 marks 成功:", marksData, "->", parsed);
          return parsed;
        } catch (e2) {
  ...
  _isFieldMarked(rowData, fieldName) {
    ...
    // 获取 marks 数据
    const marksRaw = rowData.marks || rowData.Marks || [];
    const marks = this._parseMarks(marksRaw);

    // 检查字段是否在 marks 中
    const isMarked = Array.isArray(marks) && marks.includes(fieldName);
    if (isMarked) {
      console.log(`字段 ${fieldName} 被标记为红色`, marks);
    }
    return isMarked;
  }
  ...
  getBodyCellStyle(rowIndex, rowData = null, fieldName = null) {
    ...
    // ========== 1. 优先检查行背景色（根据 log_type 字段） ==========
    if (
      rowData &&
      rowData.log_type !== undefined &&
      rowData.log_type !== null
    ) {
      // 支持 log_type 为字符串或数字
      const logType = rowData.log_type.toString();
    ...
    if (rowData && fieldName && this._isFieldMarked(rowData, fieldName)) {
      ...
      console.log(`设置红色: ${fieldName}, 行索引: ${rowIndex}`);
    }
  ...
  async export(designData, resultData, customFileName = null) {
    ...
    try {
      ...
      for (const tableKey of detailTables) {
        ...
        // ========== 打印包含 log_type 或 marks 的行数据，用于调试 ==========
        const styledRowsCount = rowsData.filter(
          (row) =>
            (row.log_type !== undefined && row.log_type !== null) ||
