class ExcelExporter {
  ...
  extractGroupNamesFromHorizontalData(resultData) {
    ...
    if (horizontalData && Array.isArray(horizontalData)) {
      ...
      // 去重处理：如果有重复的文件名，保留第一个
      const uniqueFileNames = new Map();
      for (const item of horizontalData) {
        // 优先使用 fileName，如果没有则使用 u_file_name
        let fileName = item.fileName || item.u_file_name;
        if (fileName && !uniqueFileNames.has(fileName)) {
          uniqueFileNames.set(fileName, item);
        }
      }

      // 转换为数组并建立索引映射
      const uniqueList = Array.from(uniqueFileNames.values());
      console.log(`去重后共 ${uniqueList.length} 个唯一项目`);
      ...
      uniqueList.forEach((item, index) => {
        ...
        if (fileName) {
          ...
          projectName = `项目${groupNum}`;
        }
        groupNames.set(groupNum, projectName);
        console.log(
          `项目${groupNum} 名称: ${projectName} (原始文件: ${fileName})`,
        );
      });
    } else {
  ...
  buildDynamicColumns(dynamicInfo) {
    ...
    console.log(
      "分组名称:",
      Array.from(groups.entries()).map(([num, g]) => `${num}: ${g.groupName}`),
    );
