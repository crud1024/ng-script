class ExcelExporter {
  ...
  extractGroupNamesFromHorizontalData(resultData) {
    ...
    if (horizontalData && horizontalData.length > 0) {
      ...
      // 直接使用原顺序，不去重（保持与动态字段后缀的对应关系）
      // 注意：动态字段后缀 -1 对应数组索引0，-2对应索引1，-3对应索引2
      horizontalData.forEach((item, index) => {
        ...
        if (fileName) {
          ...
          console.log(
            `映射: 后缀-${groupNum} -> "${projectName}" (原始: ${fileName})`,
          );
        } else {
          groupNames.set(groupNum, `项目${groupNum}`);
          console.log(`映射: 后缀-${groupNum} -> 项目${groupNum} (无文件名)`);
        }
      });
    } else {
